...
def collect():
    rospy.init_node('position_data_collect', anonymous=True)
    rospy.Subscriber('decaPos', Point, deca_callback)
    rospy.Subscriber('vrpn_client_node/f1car/pose', PoseStamped, vicon_callback)
    rospy.Subscriber('drive_parameters', drive_param, drive_callback)    
    file_writer = Process(target=write_to_file, args=(deca_pos, vicon_pos, drive_vals))
    file_writer.start()
    rospy.spin()
    file_writer.join()
